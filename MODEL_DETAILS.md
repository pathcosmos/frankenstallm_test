...
> **작성일:** 2026-03-11
> **목적:** FRANKENSTALLM 3B 종합 벤치마크에 사용되는 전체 11개 모델의 아키텍처, 양자화, 토크나이저, 성능 특성을 상세히 기록
> **환경:** Ubuntu Linux 6.8.0-101-generic, Ollama (CPU 추론), Python 3.12
...
| # | 모델명 | 버전 | 양자화 | 파일 크기 | 파라미터 수 | 아키텍처 | Vocab 크기 | 컨텍스트 길이 | 임베딩 차원 | 토크나이저 | 상태 |
|---|--------|------|--------|-----------|-------------|----------|------------|---------------|-------------|------------|------|
| 1 | `frankenstallm-3b-Q4_K_M` | v1 | Q4_K_M | 1.9 GB | 3.2B | LLaMA | 64,000 | 4,096 | 3,072 | SPM (결함) | **CRASH** |
| 2 | `frankenstallm-3b-Q8_0` | v1 | Q8_0 | 3.2 GB | 3.2B | LLaMA | 64,000 | 4,096 | 3,072 | SPM (결함) | **CRASH** |
| 3 | `frankenstallm-3b-f16` | v1 | F16 | 6.0 GB | 3.2B | LLaMA | 64,000 | 4,096 | 3,072 | SPM (결함) | **CRASH** |
| 4 | `frankenstallm-3b-v2-Q4_K_M` | v2 | Q4_K_M | 757 MB | 1.2B | LLaMA | 64,256 | 4,096 | 2,048 | SPM (수정) | **OK** |
| 5 | `frankenstallm-3b-v2-Q8_0` | v2 | Q8_0 | 1.2 GB | 1.2B | LLaMA | 64,256 | 4,096 | 2,048 | SPM (수정) | **OK** |
| 6 | `frankenstallm-3b-v2-f16` | v2 | F16 | 2.3 GB | 1.2B | LLaMA | 64,256 | 4,096 | 2,048 | SPM (수정) | **OK** |
| 7 | `qwen2.5:3b` | - | Q4_K_M | 1.8 GB | 3.1B | Qwen 2 | 151,936 | 32,768 | 2,048 | BPE | **OK** |
| 8 | `gemma3:4b` | - | Q4_K_M | 3.2 GB | 4.3B | Gemma 3 | 262,144 | 131,072 | 2,560 | BPE | **OK** |
| 9 | `phi4-mini` | - | Q4_K_M | 2.4 GB | 3.8B | Phi-3 | 100,352 | 131,072 | 3,072 | BPE | **OK** |
| 10 | `exaone3.5:2.4b` | - | Q4_K_M | 1.6 GB | 2.7B | EXAONE | 102,400 | 32,768 | 2,560 | BPE | **OK** |
| 11 | `llama3.2:3b` | - | Q4_K_M | 1.9 GB | 3.2B | LLaMA | 128,256 | 131,072 | 3,072 | BPE | **OK** |

### 주요 관찰 사항

- **v1 vs v2 파라미터 수 차이**: v1은 3.2B 파라미터/3,072 임베딩, v2는 1.2B 파라미터/2,048 임베딩으로 보고됨. 이는 v2가 모델 구조 자체를 축소하여 재빌드한 것임을 시사한다.
- **v2 파일 크기 대폭 감소**: 동일 양자화 기준으로 v2는 v1 대비 약 40% 수준의 파일 크기를 보인다 (Q4_K_M: 1.9GB → 757MB).
- **Vocab 크기 차이**: v1의 64,000에서 v2의 64,256으로 256개 토큰 추가 — 이는 바이트 폴백(byte fallback) 토큰을 보충한 결과이다.
- **비교 모델의 Vocab 크기**: Gemma 3가 262,144로 가장 크고, FRANKENSTALLM이 64,256으로 가장 작다. 대형 Vocab은 다국어 처리에 유리하지만 임베딩 레이어 크기가 증가한다.
